...
def system_install():
    ...
    status, output = exec_cmd("pip3 install  /usr/share/sonic/device/x86_64-quanta_ix8_dnv-r0/sonic_platform-1.0-py3-none-any.whl",1)
    if status:
        print(output)
        if FORCE == 0:
            return status
...
def install():
    if not device_found():
        ...
        print(" ix8d driver already installed....")
    return
...
def device_found():
    ret1, log1 = exec_cmd("cat /proc/modules | grep ix8d > /tmp/chkdriver.log", 0)
    ret2, log2 = exec_cmd("cat /tmp/chkdriver.log | grep ix8d", 0)
