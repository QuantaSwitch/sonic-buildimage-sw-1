...
static struct i2c_board_info ix8d_i2c_devices[] = {
	{
...
static struct platform_driver ix8d_platform_driver = {
	.driver = {
		.name = "qci-ix8d",
		.owner = THIS_MODULE,
	},
...
static struct platform_device *ix8d_device;

static struct i2c_client **g_client;
static struct i2c_client **g_client_port;
int numof_i2c_devices = 19; // num of ix8d_i2c_devices - 2 (for optoe1, optoe2)
int numof_ports = 56;

static int __init ix8d_platform_init(void)
{
	struct i2c_adapter *adapter;
	int ret, i;

	ret = platform_driver_register(&ix8d_platform_driver);
	if (ret < 0)
		return ret;

	/* Register platform stuff */
	ix8d_device = platform_device_alloc("qci-ix8", -1);
	if (!ix8d_device) {
		ret = -ENOMEM;
		goto fail_platform_driver;
	}

	ret = platform_device_add(ix8d_device);
	if (ret)
	...
	{
		g_client[0] = i2c_new_client_device(adapter, &ix8d_i2c_devices[0]);		// pca9546
		g_client[1] = i2c_new_client_device(adapter, &ix8d_i2c_devices[1]);		// pca9548
		g_client[2] = i2c_new_client_device(adapter, &ix8d_i2c_devices[16]);		// pca9546cpu
	...
	{
		g_client[3] = i2c_new_client_device(adapter, &ix8d_i2c_devices[17]);		// CPU Board Data
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[4] = i2c_new_client_device(adapter, &ix8d_i2c_devices[10]);		// CPLD_1
		g_client[5] = i2c_new_client_device(adapter, &ix8d_i2c_devices[18]);		// CPLD_4
		g_client[6] = i2c_new_client_device(adapter, &ix8d_i2c_devices[19]);		// CPLD_6
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[7] = i2c_new_client_device(adapter, &ix8d_i2c_devices[11]);		// CPLD_2
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[8] = i2c_new_client_device(adapter, &ix8d_i2c_devices[12]);		// CPLD_3
		g_client[9] = i2c_new_client_device(adapter, &ix8d_i2c_devices[2]);		// MB_BOARDINFO_EEPROM
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[10] = i2c_new_client_device(adapter, &ix8d_i2c_devices[13]);		// MB Board Data
		g_client[11] = i2c_new_client_device(adapter, &ix8d_i2c_devices[14]);		// QSFP:49~52
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[12] = i2c_new_client_device(adapter, &ix8d_i2c_devices[3]);		// pca9548_1 SFP
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[13] = i2c_new_client_device(adapter, &ix8d_i2c_devices[4]);		// pca9548_2 SFP
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[14] = i2c_new_client_device(adapter, &ix8d_i2c_devices[5]);		// pca9548_3 SFP
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[15] = i2c_new_client_device(adapter, &ix8d_i2c_devices[6]);		// pca9548_4 SFP
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[16] = i2c_new_client_device(adapter, &ix8d_i2c_devices[7]);		// pca9548_5 SFP
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[17] = i2c_new_client_device(adapter, &ix8d_i2c_devices[8]);		// pca9548_6 SFP
		i2c_put_adapter(adapter);
	}
	...
	{
		g_client[18] = i2c_new_client_device(adapter, &ix8d_i2c_devices[9]);		// pca9548_7 QSFP
		i2c_put_adapter(adapter);
	}
	...
	for(i = 65; i < 73; i ++){									// QSFP 49~56 EEPROM
		...
		{
			g_client_port[i - 17] = i2c_new_client_device(adapter, &ix8d_i2c_devices[15]);
			i2c_put_adapter(adapter);
		}
	}
	...
	for(i = 17; i < 65; i ++){									// SFP28 1~48 EEPROM
		...
		{
			adapter = i2c_get_adapter_wait(i);
			g_client_port[i - 17] = i2c_new_client_device(adapter, &ix8d_i2c_devices[20]);
			i2c_put_adapter(adapter);
		}
	}
...
fail_platform_device:
	platform_device_put(ix8d_device);

fail_platform_driver:
	platform_driver_unregister(&ix8d_platform_driver);
	return ret;
}

static void __exit ix8d_platform_exit(void)
{
	int i = 0;
	...
	platform_device_unregister(ix8d_device);
	platform_driver_unregister(&ix8d_platform_driver);
}

module_init(ix8d_platform_init);
module_exit(ix8d_platform_exit);


MODULE_AUTHOR("Jonathan Tsai <jonathan.tsai@quantatw.com>");
MODULE_VERSION("1.0");
MODULE_DESCRIPTION("Quanta IX8D Platform Driver");
MODULE_LICENSE("GPL");
